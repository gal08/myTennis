...
class VideoInteractionFrame(wx.Frame):
    ...
    def on_play(self, event):
        """
        Play the video - sends PLAY_VIDEO request, receives port, connects client.
        ...
        try:
            # Request server to start streaming for this specific video
            response = self.client._send_request('PLAY_VIDEO', {
                'video_title': self.video['title']
            })

            if response.get('status') == 'success':
                # ← THE FIX: get the port the server assigned for this video
                port = response.get('port')
                if not port:
                    wx.MessageBox(
                        "Server did not return a streaming port.",
                        "Error",
                        wx.OK | wx.ICON_ERROR,
                    )
                    return

                print(f"[GUI] Connecting to video server on port {port}")
                # Pass the correct port - each video has its own server/port
                run_video_player_client(host=DEFAULT_VIDEO_HOST, port=port)
            else:
                wx.MessageBox(
