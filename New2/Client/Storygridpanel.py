...
class StoryGridPanel(wx.Panel):
    ...
    def on_media_double_click(self, media_item):
        """
        Handle double click on story - request playback and connect to correct port.
        ...
        try:
            ...
            if response.get('status') == 'success':
                # ← THE FIX: get the port assigned for this specific story
                port = response.get('port')
                if not port:
                    self._show_error("Server did not return a streaming port.")
                    return

                print(f"[StoryGrid] Connecting to story server on port {port}")
                # ← THE FIX: each story has its own server/port - pass it!
                run_story_player_client(host=SERVER_IP, port=port)
            else:
                self._show_error(
