...
class DBManager:
    ...
    def get_video_by_title(self, title: str) -> Optional[Dict[str, Any]]:
        """
        Get video by title/filename.

        Args:
            title: Video filename to search for
        ...
        try:
            query = f'''
                ...
                WHERE filename = ?
            '''
            result = self.execute_query(query, (title,), fetch_one=True)
            ...
            print(f"[DEBUG] Error getting video by title: {e}")
            return None
