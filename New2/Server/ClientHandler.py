...
# ── Compression ───────────────────────────────────────────────────────────────
COMPRESS_LEVEL = 1          # zlib level 1 = fastest (level 9 = best ratio)
                            # For video streaming level 1 is the sweet spot:
                            # already-compressed JPEG data won't shrink much
                            # more, so speed matters more than ratio.
...
class ClientHandler:
    ...
    def handle_streaming(self):
        ...
        print(f"[ClientHandler #{self.client_id}] Streaming {self.video_path} → {self.address}")
    ...
    def _build_stream_info(self, props, audio_info, samples_per_frame, audio_proc) -> dict:
        return {
            ...
            'compressed': True,     # tells client that payloads are zlib-compressed
        }
    ...
    def _send_compressed_encrypted(self, obj) -> bool:
        """
        Serialise → zlib compress → AES encrypt → Protocol.send_bin
        Returns False if the client has disconnected.
        """
        try:
            raw = pickle.dumps(obj)                                 # serialise
            compressed = zlib.compress(raw, level=COMPRESS_LEVEL)  # compress
            if self._encryption_key:
                payload = aes_cipher.AESCipher.encrypt(            # encrypt
                    self._encryption_key, compressed
                )
            else:
                payload = compressed
